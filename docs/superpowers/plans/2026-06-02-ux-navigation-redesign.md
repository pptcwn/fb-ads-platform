...
**Goal:** Restructure navigation into 4 grouped sections, clean up Dashboard to metrics-only, and merge All Campaigns + New Campaign into a single page with a slide-in drawer.

**Architecture:** Minimal restructure — reuse existing component and page code, move logic from `all-campaigns/page.tsx` into new `campaigns/page.tsx`, embed the New Campaign form as a drawer, replace old routes with Next.js redirects. No backend changes.

**Tech Stack:** Next.js 14 App Router, React 18, TailwindCSS, TypeScript, axios
...
|---|---|---|
| `apps/web/src/components/Sidebar.tsx` | Modify | Grouped nav with section labels |
| `apps/web/src/app/dashboard/page.tsx` | Modify | Remove campaign list/modals, add quick actions row |
| `apps/web/src/app/dashboard/campaigns/page.tsx` | **Create** | Merged Campaigns table + New Campaign drawer |
| `apps/web/src/app/dashboard/all-campaigns/page.tsx` | Modify | Replace with redirect to `/dashboard/campaigns` |
| `apps/web/src/app/dashboard/campaigns/new/page.tsx` | Modify | Replace with redirect to `/dashboard/campaigns?new=1` |
...
Open `apps/web/src/components/Sidebar.tsx` and replace the entire file with:
...
Start dev server if not running: `pnpm dev` from repo root.  
Open `http://localhost:3001/dashboard` (or 3000 depending on port).  
Check: 4 group labels visible (OVERVIEW, CAMPAIGNS, AUTOMATION, INSIGHTS), 11 nav items total, Dashboard active state highlights correctly, Campaigns link goes to `/dashboard/campaigns`.

- [ ] **Step 3: Commit**

```bash
git add apps/web/src/components/Sidebar.tsx
git commit -m "feat(web): restructure sidebar into 4 grouped nav sections"
```

---

## Task 2: Redirects for Old Routes

**Files:**
- Modify: `apps/web/src/app/dashboard/all-campaigns/page.tsx`
- Modify: `apps/web/src/app/dashboard/campaigns/new/page.tsx`

- [ ] **Step 1: Replace all-campaigns page with redirect**

Replace entire contents of `apps/web/src/app/dashboard/all-campaigns/page.tsx` with:
...
- [ ] **Step 2: Replace campaigns/new page with redirect**

Replace entire contents of `apps/web/src/app/dashboard/campaigns/new/page.tsx` with:
...
Navigate to `http://localhost:3001/dashboard/all-campaigns` — should redirect to `/dashboard/campaigns` (will 404 until Task 3 creates that page, that's expected).  
Navigate to `http://localhost:3001/dashboard/campaigns/new` — should redirect to `/dashboard/campaigns?new=1`.
...
git commit -m "feat(web): redirect old campaign routes to /dashboard/campaigns"
```

---

## Task 3: New Campaigns Page with Drawer

**Files:**
- Create: `apps/web/src/app/dashboard/campaigns/page.tsx`

- [ ] **Step 1: Create the new campaigns page**

Create `apps/web/src/app/dashboard/campaigns/page.tsx` with the following content.

This file combines all logic from the old `all-campaigns/page.tsx` plus a New Campaign drawer that contains all form logic from `campaigns/new/page.tsx`.
      ...
      <div className="px-6 py-6">
        ...
        {allCampaigns.length === 0 ? (
          <div className="card p-12 text-center">
            ...
            <button onClick={openDrawer} className="btn-primary mt-4 inline-block">✨ New Campaign</button>
          </div>
        ) : (
...
- [ ] **Step 2: Verify campaigns page loads**

Navigate to `http://localhost:3001/dashboard/campaigns`.  
Check: campaign table renders, "✨ New Campaign" button visible in header, clicking it opens drawer from right, Escape key closes drawer, backdrop click closes drawer.

- [ ] **Step 3: Verify drawer form**

Open drawer → switch between Wizard and Quick mode → fill in form → submit → check table refreshes after success message.

- [ ] **Step 4: Verify ?new=1 param**

Navigate to `http://localhost:3001/dashboard/campaigns?new=1` — drawer should open automatically on page load.

- [ ] **Step 5: Commit**

```bash
git add apps/web/src/app/dashboard/campaigns/page.tsx
git commit -m "feat(web): add campaigns page with integrated new campaign drawer"
```
...
- [ ] **Step 1: Remove campaign-related state and functions**

In `apps/web/src/app/dashboard/page.tsx`, remove these state declarations (lines to find and delete):
...
Also remove the `Campaign` interface and the functions: `loadCampaigns`, `openEdit`, `saveEdit`, `confirmDelete`.

- [ ] **Step 2: Remove the Ad Accounts accordion section from JSX**

Find and remove the entire JSX block starting with:
```tsx
...
Also remove the Edit Campaign modal block and the Delete Campaign modal block at the bottom of the return statement.

- [ ] **Step 3: Add Quick Action row after Summary Stats**

Find the Summary Stats grid `<div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">` and add this block directly after its closing `</div>`:
...
Check: no campaign list or accordion, summary stats visible, quick action row visible below stats, warmup section still present, charts still render when insights loaded.

- [ ] **Step 5: Commit**

```bash
git add apps/web/src/app/dashboard/page.tsx
git commit -m "feat(web): clean up dashboard to metrics-only, add quick actions row"
```
...
**Spec coverage check:**
- ✅ Sidebar grouped into 4 sections (Task 1)
- ✅ All Campaigns removed from sidebar, replaced with Campaigns (Task 1)
- ✅ New Campaign removed from sidebar (Task 1)
- ✅ `/dashboard/all-campaigns` redirects to `/dashboard/campaigns` (Task 2)
- ✅ `/dashboard/campaigns/new` redirects to `/dashboard/campaigns?new=1` (Task 2)
- ✅ Campaigns page has table + drawer (Task 3)
- ✅ Drawer opens via button and `?new=1` param (Task 3)
- ✅ Drawer closes via X, Escape, backdrop (Task 3 — Escape listener in useEffect, backdrop onClick, X button)
- ✅ On success: close drawer + refresh table (Task 3 — setTimeout + fetchAll)
- ✅ All bulk actions preserved (Task 3)
- ✅ Clone, Ad Sets, Template, CSV preserved (Task 3)
- ✅ Dashboard has no campaign list or modals (Task 4)
- ✅ Quick actions row added (Task 4)

**Placeholder scan:** No TBD, no "implement later", all code shown in full.

**Type consistency:** `FormErrors`, `AdAccountWithCampaigns`, `CampaignItem`, `AdSetItem`, `BulkResult` defined once at top of Task 3 file, used consistently throughout. `drawerMode` typed as `DrawerMode = 'wizard' | 'quick'`. `confirmVariant` used in `ConfirmModal` matches existing prop type from `Modal.tsx`.
